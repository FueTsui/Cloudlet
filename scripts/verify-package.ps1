...
$uiPath = Join-Path $evidenceDirectory 'published-ui\visual-smoke.json'
...
$processResult = Get-Content -LiteralPath (Join-Path $evidenceDirectory 'published-process-result.json') -Raw | ConvertFrom-Json
...
$core = Get-Content -LiteralPath (Join-Path $evidenceDirectory 'core-tests.json') -Raw | ConvertFrom-Json
if ($core.failures.Count -ne 0 -or $core.passed.Count -ne 16) { throw 'Core regression evidence is missing or failed.' }
...
$output = Join-Path $evidenceDirectory 'package-verification.json'
